...
export const PUBLIC_SMB_SCREENCAST_SCHEMA = "lazuli-public-smb-screencast-v6";
...
export function derivePublicSmbTerminalProof(report) {
  ...
  if (report.sustainedPlay?.schema !== SMB_SUSTAINED_PLAY_SCHEMA_V5) {
    throw captureFailure(
      `terminal report requires ${SMB_SUSTAINED_PLAY_SCHEMA_V5}`,
    );
  }
  let sustainedPlay;
  try {
...
function beginObservedSmbSustainedWindow(
  ...
  if (report.sustainedPlay?.schema !== SMB_SUSTAINED_PLAY_SCHEMA_V5) {
    throw captureFailure(
      `observed sustained-play window requires ${SMB_SUSTAINED_PLAY_SCHEMA_V5}`,
    );
  }
  let prefix;
  try {
