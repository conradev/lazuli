  ...
  const context = {
    ...
    nextRuntimeEventCycle(includeCycleLimit, coalesceIdleAudio = false) {
      ...
      assert.equal(
        coalesceIdleAudio,
        false,
        "the AISCNT polling accelerator requires exact sample deadlines",
      );
...
test("AISCNT polling retains raw per-sample scheduler granularity", () => {
  const gate = extractFunction("aiSrcInitSampleCounterWaitWakeCycle");
  assert.match(gate, /const audioSampleCycle = nextAudioSampleCycle\(\);/);
  assert.match(gate, /const eventCycle = nextRuntimeEventCycle\(false\);/);
  assert.doesNotMatch(gate, /nextAudioInterruptCycle|coalesceIdleAudio/);
});
