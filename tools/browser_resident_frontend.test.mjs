...
test("ordinary resident runs use one fixed release batching policy", () => {
  const { POLICY } = testContext();
...
test("run results accept only actual adapter boundaries and canonical decimals", () => {
  ...
  assert.equal(validateRunResult({
    result: { boundary: "cold-install-cap", hostCalls: 0, coldInstalls: 4_096 },
...
test("ordinary run loop forwards the exact fixed batching policy", async () => {
  const { POLICY, ResidentSession } = testContext();
  const requests = [];
  const statuses = [];
  const session = new ResidentSession({
    setStatus(...status) { statuses.push(status); },
    sessionFailed(error) { assert.fail(`unexpected session failure: ${error}`); },
  }, null, "fixture", null);
  session.client = {
    request(message, responseType, timeoutMs) {
      requests.push({ message: structuredClone(message), responseType, timeoutMs });
  ...
  assert.deepEqual(statuses, [["stopped", "stopped", "fixture"]]);
});
