...
export class ResidentMachineAdapter {
  ...
  async runSlice(options = {}) {
    ...
    const cycleUpperCap = requestedCycles < RESIDENT_CYCLE_UPPER_CAP
      ? requestedCycles
      : RESIDENT_CYCLE_UPPER_CAP;
    const blockUpperCap = Math.min(requestedBlocks, RESIDENT_BLOCK_UPPER_CAP);
    ...
    try {
      for (;;) {
        ...
        const outcome = readRunOutcome(
          this.core,
          this.memory,
          this.coordinator.core_run(cycleUpperCap, blockUpperCap) >>> 0,
        );
