...
/// Offset at which resident hooks request an exact completed-instruction outer boundary.
pub const RESIDENT_HOOK_EXIT_REQUESTED_OFFSET: i32 =
    core::mem::offset_of!(ResidentControl, exit_requested) as i32;
const _: () = assert!(RESIDENT_HOOK_EXIT_REQUESTED_OFFSET == 4);
...
impl Jit {
    ...
    pub fn with_slow_memory_resident_hook_cycles() -> Self {
        Self::with_exit_mode_and_hook_cycle_offset(
            ...
            Some(RESIDENT_HOOK_CYCLE_OFFSET),
            Some(RESIDENT_HOOK_EXIT_REQUESTED_OFFSET),
            Some(browser_secondary_fastmem_config()),
            RESIDENT_HOOK_IMPORT_MODULE,
        )
    }

    /// Creates the Rust-owned resident compiler with exact hook-cycle publication.
    ///
    /// The Rust browser machine supplies only the primary 128 KiB fast-memory LUT. Its checked
    /// memory hooks own translated access and therefore must not probe the browser-only tagged
    /// sidecar that follows the fixed LUT in the legacy linear-memory layout.
    pub fn with_slow_memory_rust_resident_hook_cycles() -> Self {
        Self::with_exit_mode_and_hook_cycle_offset(
            ExitMode::ReturnExecutedWithSlowMemory,
            Some(RESIDENT_HOOK_CYCLE_OFFSET),
            Some(RESIDENT_HOOK_EXIT_REQUESTED_OFFSET),
    ...
    fn with_exit_mode(exit_mode: ExitMode) -> Self {
        Self::with_exit_mode_and_hook_cycle_offset(exit_mode, None, None, None, HOOK_IMPORT_MODULE)
    }

    fn with_exit_mode_and_hook_cycle_offset(
        exit_mode: ExitMode,
        hook_cycle_offset: Option<i32>,
        hook_exit_requested_offset: Option<i32>,
        ...
        config.hook_exit_requested_offset = hook_exit_requested_offset;
