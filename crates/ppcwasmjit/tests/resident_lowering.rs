...
use ppcwasmjit::{
    IMPORT_MODULE, Jit, MEMORY_IMPORT, RESIDENT_HOOK_EXIT_REQUESTED_OFFSET,
    RESIDENT_HOOK_IMPORT_MODULE, SECONDARY_FASTMEM_CONTROL_OFFSET, SECONDARY_FASTMEM_END_OFFSET,
};
use wasm_encoder::{
...
const WRITE_REQUEST_EXIT: u32 = 0x401c;
...
fn rust_hook_module() -> Vec<u8> {
    ...
    write.instruction(&Instruction::I32Store(memarg(0)));
    write.instruction(&Instruction::I32Const(WRITE_REQUEST_EXIT as i32));
    write.instruction(&Instruction::I32Load(memarg(0)));
    write.instruction(&Instruction::If(BlockType::Empty));
    write.instruction(&Instruction::LocalGet(0));
    write.instruction(&Instruction::I32Const(1));
    write.instruction(&Instruction::I32Store(memarg(
        RESIDENT_HOOK_EXIT_REQUESTED_OFFSET as u32,
    )));
...
#[test]
fn completed_resident_device_store_exits_before_the_following_instruction() {
    if Command::new("node").arg("--version").output().is_err() {
        eprintln!("node is unavailable; skipping WebAssembly runtime smoke test");
        return;
    }

    let block = Jit::with_slow_memory_rust_resident_hook_cycles()
        .build([stwu(4, 3, 4), addi(5, 5, 1)])
        .unwrap();
...
const [blockHex, hooksHex] = args;
const [pcOffset, r3Offset, r4Offset, r5Offset, writeCalls, writeAddress, writeValue,
       writeRequestExit, exitOffset, initialPages, maximumPages] = args.slice(2).map(Number);
const control = 0x8000;
const cpu = 0x10000;
const fastmem = 0x20000;
const startPc = 0x80006000;
const guestAddress = 0xcc008000;
const storedValue = 0x55667788;
...
view.setUint32(cpu + r3Offset, guestAddress - 4, true);
view.setUint32(cpu + r4Offset, storedValue, true);
view.setUint32(cpu + r5Offset, 9, true);
view.setUint32(writeRequestExit, 1, true);
const result = block.run(control, cpu, fastmem) >>> 0;
if (result !== 0x00020001) {
  throw new Error(`device boundary reported 0x${result.toString(16)} instead of one store`);
}
if (view.getUint32(writeCalls, true) !== 1 ||
    view.getUint32(writeAddress, true) !== guestAddress ||
    view.getUint32(writeValue, true) !== storedValue) {
  throw new Error("device store did not commit exactly once");
}
if (view.getUint32(cpu + pcOffset, true) !== startPc + 4 ||
    view.getUint32(cpu + r3Offset, true) !== guestAddress ||
    view.getUint32(cpu + r5Offset, true) !== 9) {
  throw new Error("device boundary failed to retire update-form state or executed the trailing sentinel");
}
if (view.getUint32(control + exitOffset, true) !== 1) {
  throw new Error("device hook did not retain its outer-boundary request");
}
"#;
    ...
    let output = Command::new("node")
        .args([
            ...
            &WRITE_REQUEST_EXIT.to_string(),
            &RESIDENT_HOOK_EXIT_REQUESTED_OFFSET.to_string(),
...
#[test]
fn rust_owned_resident_uses_memory_hooks_without_touching_a_sidecar() {
    if Command::new("node").arg("--version").output().is_err() {
        eprintln!("node is unavailable; skipping WebAssembly runtime smoke test");
        return;
    }
...
// This is the exact translated MEM1 target used by Wario's common exception-save path. The
// primary-hit phase models Rust's permission-aware BAT table; the later phases clear that entry
// and model an exactly-sized logical primary LUT with no secondary sidecar.
const guestAddress = 0x80349ac8;
const directPage = 0x300000;
const memory = new WebAssembly.Memory({
  initial: Number(args[17]),
  maximum: Number(args[18]),
});
const view = new DataView(memory.buffer);
const primaryLutBytes = Number(args[19]);
const formerSidecarEnd = Number(args[20]);
const guard = new Uint8Array(
  memory.buffer,
  fastmem + primaryLutBytes,
  formerSidecarEnd - primaryLutBytes,
);
guard.fill(0xa5);
const expectedGuard = guard.slice();
...
const primaryEntry = fastmem + (guestAddress >>> 17) * 4;
const pageOffset = guestAddress & 0x1ffff;
view.setUint32(primaryEntry, directPage, true);
view.setUint32(directPage + pageOffset, readValue, false);
reset();
const fastCompleted = block.exports.run(0, cpu, fastmem) >>> 0;
if (fastCompleted !== (expectedExecuted >>> 0)) {
  throw new Error(`primary fastmem path returned 0x${fastCompleted.toString(16)}`);
}
if (view.getUint32(readCalls, true) !== 0 || view.getUint32(writeCalls, true) !== 0) {
  throw new Error("primary fastmem hit crossed a scalar Rust hook");
}
if (view.getUint32(directPage + pageOffset + 4, false) !== (readValue >>> 0) ||
    view.getUint32(cpu + r4Offset, true) !== (readValue >>> 0) ||
    view.getUint32(cpu + pcOffset, true) !== ((startPc + 8) >>> 0)) {
  throw new Error("primary fastmem path changed value, store, or PC semantics");
}
view.setUint32(primaryEntry, 0, true);
...
for (let index = 0; index < guard.length; index++) {
  if (guard[index] !== expectedGuard[index]) {
    throw new Error(`Rust-owned resident touched unallocated sidecar byte ${index}`);
  }
}
"#;
    ...
    let output = Command::new("node")
        .args([
            ...
            &SECONDARY_FASTMEM_END_OFFSET.to_string(),
        ])
        .output()
        .unwrap();
    assert!(
