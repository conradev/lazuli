...
impl GxMemory for ResidentMem1<'_> {
    fn read_exact(&mut self, address: u32, destination: &mut [u8]) -> Result<(), GxMemoryError> {
        let start = usize::try_from(address).map_err(|_| GxMemoryError::OutOfBounds)?;
        let end = start
...
impl BrowserMachine {
    ...
    pub fn from_system(mut system: System) -> Option<Self> {
        // The resident GX loop is now the sole CP consumer. These callbacks are installed by the
        // legacy System constructor and must never race the Rust FIFO/materialization pipeline.
...
mod wasm_abi {
    ...
    pub extern "C" fn user_0_22(context: u32) {
        state_hook(context, ResidentHookBoundary::TimeOnly, |machine| {
            machine.system.lazy.last_updated_dec = machine.system.scheduler.elapsed();
            machine
                .system
                .scheduler
                .cancel(lazuli::system::System::decrementer_overflow);
            let decrementer = machine.system.cpu.supervisor.misc.dec;
            if machine
                .system
                .scheduler
                .try_schedule(
                    u64::from(decrementer),
                    lazuli::system::System::decrementer_overflow,
                )
                .is_ok()
            {
                HookResult::COMPLETE
...
mod tests {
    ...
    #[test]
    fn browser_machine_cancels_both_legacy_gx_callbacks_at_construction() {
        ...
        let machine = BrowserMachine::from_system(system).unwrap();
        assert!(
            !machine
                .system
                .scheduler
    ...
    fn resident_time_hooks_observe_exact_two_block_cycle_and_service_the_authored_event() {
        ...
        machine
            .observe_resident_hook(|machine| {
                ...
                machine
                    .system
                    .scheduler
                    .cancel(System::decrementer_overflow);
                machine
                    .system
                    .scheduler
                    .try_schedule(7, System::decrementer_overflow)
            })
            .unwrap()
            .unwrap();
        assert_eq!(machine.system.lazy.last_updated_dec, 142);
        assert_eq!(machine.system.scheduler.canonical_elapsed(), 119);
        assert_eq!(machine.system.scheduler.until_next(), Some(30));
        assert!(!machine.system.scheduler.is_observing());
        assert!(machine.resident_context.control.should_exit());
        ...
        assert_eq!(machine.system.scheduler.until_next(), Some(7));

        machine.system.scheduler.advance(7);
        assert_eq!(
            machine.service_due_resident_events(),
            ResidentEventService::Complete
        );
        assert_eq!(machine.system.cpu.supervisor.misc.dec, 0);
        ...
        let result = machine
            .observe_resident_hook(|machine| {
                ...
                if machine
                    .system
                    .scheduler
                    .try_schedule(10, System::decrementer_overflow)
                    .is_err()
                {
                    machine.publish_machine_exit(ResidentMachineExitDetail::HookScheduleRejected);
                    HookResult {
