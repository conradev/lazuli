...
impl System {
    ...
    pub fn launch_hle_executable(&mut self, entry: Address) {
        ...
        *msr = Default::default();
        ...
        self.cpu.pc = entry;
        self.refresh_data_fastmem_luts();
    }

    /// Refreshes the primary translated-data fast path from one coherent CPU-register snapshot.
    ///
    /// The retained table is signature-cached by the memory subsystem, so unrelated MSR writes
    /// and DR-only transitions do not rescan the effective address space.
    pub fn refresh_data_fastmem_luts(&mut self) -> bool {
        let registers = TranslationRegisters::from_cpu(&self.cpu);
        self.mem.refresh_data_fastmem_luts(&registers)
    }
...
mod tests {
    ...
    fn hle_executable_launch_authors_the_complete_translation_transition() {
        ...
        system.write_phys_slow(Address(0x1000), 0x1234_5678u32);
        assert_eq!(
            system.read_fast::<u32>(Address(0x8000_1000)),
            Some(0x1234_5678)
        );
        assert_eq!(
            system.read_fast::<u32>(Address(0xc000_1000)),
            Some(0x1234_5678)
        );
        assert!(system.write_fast(Address(0xc000_1000), 0xaabb_ccddu32));
        assert_eq!(system.read_phys_slow::<u32>(Address(0x1000)), 0xaabb_ccdd);
    }
