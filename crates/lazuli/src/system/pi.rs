...
pub fn check_interrupts(sys: &mut System) {
    // Resident semantic hooks temporarily observe an instruction-start cycle before the
    // dispatcher has authenticated and committed that instruction's work. Device code may
    // refresh PI levels in this scope, but mutating PC/MSR/SRR would let the live translated
    // block continue in exception state. The Rust outer boundary samples again after commit.
    if sys.scheduler.is_observing() {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn resident_instruction_observation_defers_pi_until_the_outer_boundary() {
        let mut system = test_system();
        system.gpu.cmd.control = Control::from_bits(0x0005);
        system.gpu.cmd.fifo.high_mark = 0x40;
        system.gpu.cmd.fifo.distance = 0x60;
        ...
        system.processor.mask.set_sources(mask);
        system.cpu.supervisor.config.msr.set_exception_prefix(false);
        restore_external_interrupts(&mut system, 0x8000_5100);

        let observation = system.scheduler.begin_observation(0).unwrap();
        check_interrupts(&mut system);
        check_interrupts_excluding_pixel_engine(&mut system);
        assert_eq!(system.cpu.pc, Address(0x8000_5100));
        assert_eq!(system.cpu.supervisor.exception.srr[0], 0);
        system.scheduler.end_observation(observation).unwrap();

        check_interrupts_excluding_pixel_engine(&mut system);
        assert_eq!(system.cpu.pc, Address(0x0000_0500));
        assert_eq!(system.cpu.supervisor.exception.srr[0], 0x8000_5100);
    }
