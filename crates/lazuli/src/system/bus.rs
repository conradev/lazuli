...
pub enum ResidentDataRead<P> {
    Complete {
        value: P,
        target: DataAccessTarget,
        serial_service: Option<si::SerialServiceSummary>,
    },
    EfbPeek {
        physical: u32,
    },
...
impl System {
    ...
    fn synchronize_resident_mmio_at(
        &mut self,
        observed_cycle: u64,
    ) -> Result<si::SerialServiceSummary, ResidentMmioError> {
        ...
        let serial_service = si::service_due(self, timing, observed_cycle, &mut deadlines)
    ...
    ) -> Result<(P, DataAccessTarget, Option<si::SerialServiceSummary>), ResidentDataAccessError>
    {
        let mapping = self
            .translate_data_range_mmu(
                addr,
        ...
        if target == DataAccessTarget::Mmio {
            ...
            let serial_service = self
                .synchronize_resident_mmio_at(observed_cycle)
                .map_err(ResidentDataAccessError::Mmio)?;
            return Ok((
                self.read_mmio_resident(mapping.physical as u16),
                target,
                Some(serial_service),
            ));
        }
        Ok((self.read_phys_slow(Address(mapping.physical)), target, None))
    }
    ...
    fn read_slow_result_classified_at_deferred<P: Primitive>(
        ...
        if target == DataAccessTarget::Mmio {
            ...
            return Ok(ResidentDataRead::Complete {
                ...
                target,
                serial_service: Some(serial_service),
            });
        }
    ...
    ) -> Result<(P, DataAccessTarget, Option<si::SerialServiceSummary>), ResidentDataAccessError>
    {
        self.read_fast(addr).map_or_else(
            ...
            |value| Ok((value, DataAccessTarget::Memory, None)),
        )
    }
    ...
    fn write_slow_result_classified_at_with<P: Primitive>(
        ...
        observed_cycle: u64,
        device_policy: ResidentDevicePolicy,
    ) -> Result<(DataAccessTarget, Option<si::SerialServiceSummary>), ResidentDataAccessError> {
        let mapping = self
            .translate_data_range_mmu(
                addr,
                size_of::<P>() as u64,
    ...
    pub fn write_result_classified_at<P: Primitive>(
        ...
        if self.write_fast(addr, value) {
            Ok((DataAccessTarget::Memory, None))
        } else {
...
mod tests {
    ...
    fn pixel_alpha_read_register_retains_raw_mode_and_exposes_canonical_mode() {
        ...
        assert_eq!(
            ...
            Ok((DataAccessTarget::Mmio, Some(Default::default())))
        );
    ...
    #[test]
    fn translated_fastmem_preserves_permissions_and_first_bat_priority() {
        let mut sys = test_system();
        let effective = Address(0x9000_0020);
        sys.cpu
            .supervisor
        ...
        assert!(sys.refresh_data_fastmem_luts());
        sys.write_phys_slow(Address(0x20), 0x1234_5678u32);

        assert_eq!(sys.read_fast::<u32>(effective), Some(0x1234_5678));
    ...
    #[test]
    fn translated_fastmem_refreshes_pr_and_dbat_identity_without_stale_pages() {
        let mut sys = test_system();
        let user_effective = Address(0x9000_0040);
        let supervisor_effective = Address(0xa000_0040);
        sys.cpu
            .supervisor
            .config
            .msr
            .set_data_addr_translation(true);
        sys.cpu.supervisor.memory.dbat[0] = bat(0x9000_0001, 0x0000_0002);
        sys.cpu.supervisor.memory.dbat[1] = bat(0xa000_0002, 0x0000_0002);
        let dbats = sys.cpu.supervisor.memory.dbat.clone();
        sys.mem.build_data_bat_lut(&dbats);
        sys.write_phys_slow(Address(0x40), 0x1020_3040u32);

        sys.cpu.supervisor.config.msr.set_user_mode(true);
        assert!(sys.refresh_data_fastmem_luts());
        assert_eq!(sys.read_fast::<u32>(user_effective), Some(0x1020_3040));
        assert_eq!(sys.read_fast::<u32>(supervisor_effective), None);
        assert!(!sys.refresh_data_fastmem_luts());

        sys.cpu.supervisor.config.msr.set_user_mode(false);
        assert!(sys.refresh_data_fastmem_luts());
        assert_eq!(sys.read_fast::<u32>(user_effective), None);
        assert_eq!(
            sys.read_fast::<u32>(supervisor_effective),
            Some(0x1020_3040)
        );

        sys.cpu.supervisor.memory.dbat[1] = Bat::default();
        let dbats = sys.cpu.supervisor.memory.dbat.clone();
        sys.mem.build_data_bat_lut(&dbats);
        assert_eq!(sys.read_fast::<u32>(supervisor_effective), None);
        assert!(sys.refresh_data_fastmem_luts());
    ...
    #[test]
    fn translated_fastmem_maps_only_complete_mem1_pages() {
        let mut sys = test_system();
        sys.cpu
            .supervisor
            .config
        ...
        for (index, physical) in [
            0x0c00_0000,
            L2C_START,
            crate::system::mem::IPL_START,
            RAM_LEN as u32,
        ]
        .into_iter()
        .enumerate()
        {
            let effective = 0x9000_0000 + (index as u32) * FASTMEM_PAGE_BYTES;
            sys.cpu.supervisor.memory.dbat[index] = bat(effective | 2, physical | 2);
        }
        let dbats = sys.cpu.supervisor.memory.dbat.clone();
        sys.mem.build_data_bat_lut(&dbats);
        assert!(sys.refresh_data_fastmem_luts());
        ...
        for index in 0..4u32 {
            let effective = Address(0x9000_0000 + index * FASTMEM_PAGE_BYTES);
            assert_eq!(sys.read_fast::<u32>(effective), None);
            assert!(!sys.write_fast(effective, 0xdead_beefu32));
        }
        ...
        sys.mem.build_data_bat_lut(&dbats);
        assert!(sys.refresh_data_fastmem_luts());
        let page_edge = Address(0x9000_0000 + FASTMEM_PAGE_BYTES - 2);
        assert_eq!(sys.read_fast::<u32>(page_edge), None);
        assert!(!sys.write_fast(page_edge, 0x5566_7788u32));
    }
    ...
    fn resident_aid_word_and_halfword_mmio_use_exact_deadlines_without_callbacks() {
        ...
        assert_eq!(
            ...
            Ok((2, DataAccessTarget::Mmio, Some(Default::default())))
        );
        assert_eq!(
    ...
    fn resident_aram_dma_halfword_programming_triggers_low_lane_and_w1c_completes() {
        ...
        assert_eq!(
            ...
            Ok((0x0820, DataAccessTarget::Mmio, Some(Default::default()),))
        );
        assert!(!sys.dsp.control.aram_dma_ongoing());
