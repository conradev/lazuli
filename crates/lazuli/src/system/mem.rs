...
use crate::system::mmu::{BatPair, BatResolution, MSR_DR, MSR_PR, Mpc750Mmu, TranslationRegisters};
...
struct DataFastmemSignature {
    user_mode: bool,
    bats: [BatPair; 4],
}

impl DataFastmemSignature {
    fn from_registers(registers: &TranslationRegisters) -> Self {
        Self {
            user_mode: registers.msr & MSR_PR != 0,
            bats: registers.data_bats,
        }
    }
}
...
pub struct Memory {
    ...
    data_fastmem_signature: Option<DataFastmemSignature>,
    data_translation_lut: Box<TranslationLut>,
    inst_translation_lut: Box<TranslationLut>,
}

fn translated_mem1_fastmem_pointer(
    ram: *mut u8,
    registers: &TranslationRegisters,
    effective: u32,
    write: bool,
) -> Option<NonNull<u8>> {
    let BatResolution::Mapped(mapping) = Mpc750Mmu::resolve_data_bat(registers, effective, write)
    else {
        return None;
    };
    let physical = mapping.physical;
    if !physical.is_multiple_of(FASTMEM_PAGE_BYTES)
        || physical
            .checked_add(FASTMEM_PAGE_BYTES)
            .is_none_or(|end| end > RAM_LEN as u32)
    {
        return None;
    }

    // SAFETY: The complete 128 KiB page was checked above to be inside the stable MEM1 backing.
    NonNull::new(unsafe { ram.add(physical as usize) })
}
...
impl Memory {
    ...
    pub fn build_data_bat_lut(&mut self, dbats: &[Bat; 4]) {
        ...
        // Fail closed while the new DBAT state is being published. The System-level transition
        // seam repopulates these tables from one coherent register snapshot immediately after
        // this legacy translation table has been rebuilt.
        self.data_fastmem_lut_logical_read.fill(None);
        self.data_fastmem_lut_logical_write.fill(None);
        self.data_fastmem_signature = None;
    ...
    /// Refreshes the permission-aware primary LUT for the current translated data address space.
    ///
    /// The exact MMU BAT resolver preserves first-match, PR, and PP behavior. Only complete
    /// 128 KiB pages backed by MEM1 are published; hashed pages, MMIO, IPL, locked cache, and
    /// partial pages remain on the checked Rust path. DR is forced for this retained logical
    /// table because real mode selects the separate physical LUT at dispatch time.
    pub fn refresh_data_fastmem_luts(&mut self, registers: &TranslationRegisters) -> bool {
        let signature = DataFastmemSignature::from_registers(registers);
        if self.data_fastmem_signature == Some(signature) {
            return false;
        }

        let mut translated = *registers;
        translated.msr |= MSR_DR;
        self.data_fastmem_lut_logical_read.fill(None);
        self.data_fastmem_lut_logical_write.fill(None);

        let ram = self.ram.as_ptr();
        for logical_base in 0..PAGES_COUNT {
            let effective = (logical_base as u32) << 17;
            self.data_fastmem_lut_logical_read[logical_base] =
                translated_mem1_fastmem_pointer(ram, &translated, effective, false);
            self.data_fastmem_lut_logical_write[logical_base] =
                translated_mem1_fastmem_pointer(ram, &translated, effective, true);
        }
        self.data_fastmem_signature = Some(signature);
        true
    }
    ...
    /// write-safe table keeps read-only or non-MEM1 pages on the checked Rust hook path instead of
    /// allowing a generated store to reuse a read-only pointer.
    #[inline(always)]
