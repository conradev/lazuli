...
use crate::system::si::SerialServiceSummary;
...
pub struct MemoryHookResult {
    ...
    /// Exact SI work performed while synchronizing a device access at its observed cycle.
    /// The browser machine must retain this until the enclosing dispatcher report commits.
    pub serial_service: Option<SerialServiceSummary>,
}
...
impl MemoryHookResult {
    ...
    fn completed(
        ...
        serial_service: Option<SerialServiceSummary>,
    ) -> Self {
        Self {
            result,
...
impl MachineRuntimeHooks {
    ...
    pub fn read_slow_classified<P: Primitive>(
        ...
        match system.read_slow_result_classified(addr) {
            Ok((read, target)) => {
                *value = read;
                MemoryHookResult::completed(HookResult::COMPLETE, target, None)
            }
    ...
    pub fn read_slow_classified_at<P: Primitive>(
        ...
        match system.read_slow_result_classified_at(addr, observed_cycle) {
            Ok((read, target, serial_service)) => {
                *value = read;
                Ok(MemoryHookResult::completed(
                    HookResult::COMPLETE,
                    target,
                    serial_service,
                ))
    ...
    pub fn write_slow_classified<P: Primitive>(
        ...
        match system.write_slow_result_classified(addr, value) {
            Ok(target) => MemoryHookResult::completed(HookResult::COMPLETE, target, None),
            Err(fault) => MemoryHookResult::resident(record_data_access_fault(system, addr, fault)),
        }
    }
    ...
    pub fn write_slow_classified_at<P: Primitive>(
        ...
        match system.write_slow_result_classified_at(addr, value, observed_cycle) {
            Ok((target, serial_service)) => Ok(MemoryHookResult::completed(
                HookResult::COMPLETE,
                target,
                serial_service,
            )),
        ...
        *size = 0;
        ...
        let read = match ty {
            QuantizedType::U8 => system
                ...
                .map(|(x, target, serial_service)| (x as f64, target, serial_service)),
            ...
            QuantizedType::Float => system
                ...
                .map(|(x, target, serial_service)| {
                    (f32::from_bits(x) as f64, target, serial_service)
                }),
        ...
        let (read, target, serial_service) = match read {
            Ok(read) => read,
    ...
    pub fn write_quantized_classified_at(
        ...
        *size = 0;
        ...
        let (target, serial_service) = match result {
            Ok(result) => result,
            Err(ResidentDataAccessError::Access(fault)) => {
                return Ok(MemoryHookResult::resident(record_data_access_fault(
                    system, addr, fault,
    ...
    pub fn msr_changed(&mut self, system: &mut System) -> HookResult {
        system.refresh_data_fastmem_luts();
...
mod tests {
    ...
    #[test]
    fn data_fastmem_hooks_publish_raw_luts_without_reallocating_them() {
        let mut system = test_system();
        let mut hooks = MachineRuntimeHooks::new(&system.cpu);
        let page = (0x9000_0000u32 >> 17) as usize;
        let read_lut = system.mem.data_fastmem_lut_logical_read().as_ptr();
        let write_lut = system.mem.data_fastmem_lut_logical_write().as_ptr();
        let physical_write_lut = system.mem.data_fastmem_lut_physical_write().as_ptr();
        assert_eq!(system.mem.resident_fastmem_write_lut_ptr(true), write_lut);
        assert_eq!(
            system.mem.resident_fastmem_write_lut_ptr(false),
            physical_write_lut
        );
        ...
        assert!(system.mem.data_fastmem_lut_logical_read()[page].is_some());
        assert!(system.mem.data_fastmem_lut_logical_write()[page].is_some());
        ...
        assert!(system.mem.data_fastmem_lut_logical_read()[page].is_none());
        assert!(system.mem.data_fastmem_lut_logical_write()[page].is_none());

        system.cpu.supervisor.memory.dbat[0] = bat(0x9000_0001, 0x0000_0001);
        assert_eq!(
            MachineRuntimeHooks::data_bat_changed(&mut system).outcome,
            HookOutcome::Complete
        );
        ...
        assert!(system.mem.data_fastmem_lut_logical_write()[page].is_none());
        assert_eq!(
            system.mem.data_fastmem_lut_logical_read().as_ptr(),
            read_lut
        );
        assert_eq!(
            system.mem.data_fastmem_lut_logical_write().as_ptr(),
            write_lut
        );
        assert_eq!(system.mem.resident_fastmem_write_lut_ptr(true), write_lut);
    }
